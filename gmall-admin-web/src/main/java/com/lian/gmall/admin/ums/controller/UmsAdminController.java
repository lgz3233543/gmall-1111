...
import com.lian.gmall.admin.ums.vo.UmsAdminLoginParam;
import com.lian.gmall.admin.ums.vo.UmsAdminParam;
import com.lian.gmall.admin.utils.JwtTokenUtil;
...
/**
 * 使用 JSR303 的三大步骤
 * 1、给需要校验数据的 JavaBean 标注校验注解
 * 2、告诉 SpringBoot，这个需要校验 @Valid
 *     SpringMvc 进入方法之前，确定参数值的时候就会进行校验，如果校验出错 Controller 代码不会执行
 * 3、如何感知校验成功还是失败
 *      只需要给开启了校验的 JavaBean 参数后面，紧跟着一个 BindingResult 对象就可以获取到校验结果；
 *          只要有 BindingResult，即使校验错了，方法也会执行。
 */
